...
Specify only predictors needed for 3e
...
preds.df <- as.data.frame(model_3e)
...
#apply that function
mod3e_resid <- residuals(preds.df$yhat_1, preds.df$y_1)

#plot the errors in a density plot

resid_plot <- plot(density(mod3e_resid), 
                          main = "Absolute Value of Residuals for Mot-Persistence Model",
                          xlab = "Residuals",
                          ylab = "Density")

##Compare the above area under the curve /ROC to baseline model
ROC_3e <-roc(preds.df$y_1, preds.df$yhat_1)
  #ROC takes Actual
  #then predictor

ROC_3e_plot <- plot(ROC_3e, print.auc = TRUE)

```

#Variance Explained 
```{r}
##Sum of squares for the regression
# regss <- sum(
#   ( preds.df$yhat_1 - mean(preds.df$yhat_1) )^2
#   )
# 
# #Sum of squares for the Total
# tss <- sum((preds.df$y_1 - mean(preds.df$y_1))^2)
# 
# rsq <- regss/tss
# rsq
