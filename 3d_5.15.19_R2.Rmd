...
```
Specify only predictors needed for 3d
```{r}
colnames(RFdata)
...
preds.df <- as.data.frame(model_3d)
...
#apply that function
mod3d_resid <- residuals(preds.df$yhat_1, preds.df$y_1)

#plot the errors in a density plot

resid_plot <- plot(density(mod3d_resid), 
                          main = "Absolute Value of Residuals for Mot-Persistence Model",
                          xlab = "Residuals",
                          ylab = "Density")

##Compare the above area under the curve /ROC to baseline model
ROC_3d <-roc(preds.df$y_1, preds.df$yhat_1)
  #ROC takes Actual
  #then predictor

ROC_3d_plot <- plot(ROC_3d, print.auc = TRUE)
