...
Specify only variables needed for 3b
```{r}
colnames(RFdata)
...
preds.df <- as.data.frame(model_3b)
...
#apply that function
mod3b_resid <- residuals(preds.df$yhat_1, preds.df$y_1)

#plot the errors in a density plot

resid_plot <- plot(density(mod3b_resid), 
                          main = "Absolute Value of Residuals for Mot-Persistence Model",
                          xlab = "Residuals",
                          ylab = "Density")

##Compare the above area under the curve /ROC to baseline model
ROC_3b <-roc(preds.df$y_1, preds.df$yhat_1)
  #ROC takes Actual
  #then predictor

ROC_3b_plot <- plot(ROC_3b, print.auc = TRUE)
