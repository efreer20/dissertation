...
Specify only predictors needed for 3c
...
preds.df <- as.data.frame(model_3c)
...
#apply that function
mod3c_resid <- residuals(preds.df$yhat_1, preds.df$y_1)

#plot the errors in a density plot

resid_plot <- plot(density(mod3c_resid), 
                          main = "Absolute Value of Residuals for Mot-Persistence Model",
                          xlab = "Residuals",
                          ylab = "Density")

##Compare the above area under the curve /ROC to baseline model
ROC_3c <-roc(preds.df$y_1, preds.df$yhat_1)
  #ROC takes Actual
  #then predictor

ROC_3c_plot <- plot(ROC_3c, print.auc = TRUE)
